...
- [ ] **Status bar showing connection to current provider and model** — persistent UI strip (likely bottom of the main window or the runner dock) surfacing: the active provider (`Ollama` / `OpenAI-compatible`), the endpoint host:port, the current model id, and a live connection health pip (green / amber / red). Today this info is only visible inside the Settings sheet, so a user whose local server crashed mid-batch has no immediate signal — they see "Retrying…" messages but nothing identifies which server they're actually hitting. The health pip should reuse the existing `preflight` machinery (already provider-agnostic via `makeCurrentClient()`) and re-probe on a slow cadence (e.g. every 30 s while idle, every 5 min while running) so it stays accurate without spamming the server. Clicking the strip should open the Settings sheet pre-scrolled to the connection card.
- [ ] **Preflight is Ollama-hardcoded when provider is OpenAI-compatible** — `ProcessingEngine.runPreflight()` already routes through `makeCurrentClient()`, so the HTTP probe itself hits the right endpoint. But two things still leak Ollama-only assumptions: (1) `PreflightSheet` is entirely Ollama-themed — copy says "Start Ollama", suggests `brew install ollama` / `ollama pull …`, and is always passed `engine.connection.baseURL` (the Ollama URL) regardless of `apiProvider`; (2) `applyConfigChange()` refreshes models but does NOT re-run preflight, so a stale Ollama-failure sheet lingers after the user switches providers in Settings. Fix: branch `PreflightSheet` copy + fix-commands + `baseURL` on `apiProvider` (and hide "Start Ollama" when provider is OpenAI-compatible); add `await runPreflight()` at the end of `applyConfigChange()`.
