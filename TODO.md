...
- [ ] **Persist the original (pre-PhotoSnail) description across re-processes** — Stream C (v0.1.2) preserves the user's existing text on FIRST touch by reading the pre-description and prepending it before our payload with a `\n\n---\n\n` separator. But on a SECOND touch (re-process), the stored description already contains a sentinel, so `Sentinel.containsAnySentinel` returns true and we overwrite the whole thing — including the user's preserved portion. Two paths to fix: (a) parse the existing description for the `\n\n---\n\n` separator and treat anything before it as the user's text on every write, OR (b) snapshot the original into a new `original_user_description` schema column at first touch and read from there forever after. (b) is more robust (handles users editing the prefix later) but needs a schema bump and a one-time backfill heuristic for already-processed assets where we don't know what was there originally.
