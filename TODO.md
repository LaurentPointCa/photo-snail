...
- [ ] **Auto-Start toggle should reflect "Waiting for lock" in the paused state too** — today the primaryButton swaps to "Waiting for lock…" only when state is `.idle` or `.finished` AND `autoStartWhenLocked` is on. If the user pauses a running batch with the toggle on, the button shows "Resume" — but on next lock, the watcher will auto-start anyway. Either: extend the Waiting-for-lock label to `.paused` when the toggle is on (so the user knows the lock will fire it), OR have the watcher respect `.paused` as "user explicitly paused, don't auto-resume on lock." Need to decide which behavior is right before changing the label.
