...
- [ ] **Standardize on "PhotoSnail" (one word) everywhere** — tester noted inconsistency and suggested "Photo Snail" would be more classic-Mac; decision is the opposite: pick `PhotoSnail` and make it consistent across the app name, readme, release notes, install.sh output, about box, and any UI strings. Audit all occurrences of "Photo Snail" / "Photo-Snail" and normalize.
- [ ] **Ollama preflight check — in install.sh AND at every app startup** — install.sh should verify Ollama is installed, running, and the configured model is pulled; print the exact shell commands needed to fix whatever's missing (no silent auto-fix). The GUI/CLI should run the same check at every startup and surface a blocking prompt if Ollama is missing/down/model not pulled, with the same copy-pasteable fix commands.
- [ ] **Rename "Start" → "Start Queue" and "Pending" → "Queued"** — the primary action button and the library counter should use the same noun so the UI is internally consistent. Audit all strings (and localizations — 8 languages) for both terms.
- [ ] **Empty queue on first launch + explicit "Add to queue" button** — stop auto-enrolling the entire library. Fresh install = empty queue. User picks photos (or "all") and clicks Add to queue. Also: **remove the "Re-Process" button** — adding an already-processed photo to the queue simply reprocesses it. One action, not two.
- [ ] **"Process now" button for a single selected photo** — works only when exactly one photo is selected. Replaces the current workaround (Reprocess → pause after 1). Bypasses or front-of-queues the selected photo and runs it immediately.
- [ ] **Preserve existing descriptions when sentinel is absent** — if the asset has a description and it does NOT carry a PhotoSnail sentinel, keep the original text and append ours after a separator (so PhotoSnail's prose comes after the user's original). Only overwrite freely when the description was written by us (sentinel present). Today we treat missing-sentinel as writable — that's the regression to fix.
- [ ] **Lower Ollama's process priority during batch runs** — the tester's machine was unusable (core pegged at 98%, "so sluggish") during processing. Investigate: renice the Ollama process, use QoS hints on our HTTP requests, or expose a toggle. Goal: interactive apps stay responsive while the batch grinds in the background.
- [ ] **"Auto-start when Mac is locked" toggle** — placed below the Start Queue button. When on: queue auto-starts on lock and auto-pauses on unlock. Targets desktop users who leave the machine running for weeks and don't want processing to interfere with active use.
