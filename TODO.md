...
- [x] **View & clean up accumulated descriptions from multiple runs** — shipped as the Tools menu (Scan multi-segment / Clean multi-segment / Scan preserved). Sentinel-count + separator-count detection heuristic, streaming findings list, dry-run by default with explicit Apply toggle on the clean tool, per-row "Show in library" button + context menu. Routes through `PhotosScripter` for write-back.
