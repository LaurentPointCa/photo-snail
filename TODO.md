...
## External tester feedback (v0.1.1, 2026-04-14)

_First external tester ran v0.1.1 on a 39,406-photo library. Captures below are the actionable items agreed with the user — naming/UX consistency, startup hygiene, queue semantics, and "don't clobber what was already there" defaults. Items raised but explicitly declined are omitted._

- [x] **Standardize on "PhotoSnail" (one word) everywhere** — tester noted inconsistency and suggested "Photo Snail" would be more classic-Mac; decision is the opposite: pick `PhotoSnail` and make it consistent across the app name, readme, release notes, install.sh output, about box, and any UI strings. Audit all occurrences of "Photo Snail" / "Photo-Snail" and normalize.
- [x] **Ollama preflight check — in install.sh AND at every app startup** — install.sh should verify Ollama is installed, running, and the configured model is pulled; print the exact shell commands needed to fix whatever's missing (no silent auto-fix). The GUI/CLI should run the same check at every startup and surface a blocking prompt if Ollama is missing/down/model not pulled, with the same copy-pasteable fix commands.
- [x] **Rename "Start" → "Start Queue" and "Pending" → "Queued"** — the primary action button and the library counter should use the same noun so the UI is internally consistent. Audit all strings (and localizations — 8 languages) for both terms.
- [x] **Empty queue on first launch + explicit "Add to queue" button** — stop auto-enrolling the entire library. Fresh install = empty queue. User picks photos (or "all") and clicks Add to queue. Also: **remove the "Re-Process" button** — adding an already-processed photo to the queue simply reprocesses it. One action, not two.
- [x] **"Process now" button for a single selected photo** — works only when exactly one photo is selected. Replaces the current workaround (Reprocess → pause after 1). Bypasses or front-of-queues the selected photo and runs it immediately.
- [x] **Preserve existing descriptions when sentinel is absent** — if the asset has a description and it does NOT carry a PhotoSnail sentinel, keep the original text and append ours after a separator (so PhotoSnail's prose comes after the user's original). Only overwrite freely when the description was written by us (sentinel present). Today we treat missing-sentinel as writable — that's the regression to fix.
- [x] **Lower Ollama's process priority during batch runs** — the tester's machine was unusable (core pegged at 98%, "so sluggish") during processing. Investigate: renice the Ollama process, use QoS hints on our HTTP requests, or expose a toggle. Goal: interactive apps stay responsive while the batch grinds in the background.
- [x] **"Auto-start when Mac is locked" toggle** — placed below the Start Queue button. When on: queue auto-starts on lock and auto-pauses on unlock. Targets desktop users who leave the machine running for weeks and don't want processing to interfere with active use.
- [x] **Lock UI while loading from Photos.app** — the initial library enumeration takes a long time on large libraries (the tester saw ~20k photos take multiple minutes, one core at 98%, throttled by Photos.app API). Lock the UI for the duration and show a status line like "Loading data from Photos.app…" so it's obvious the app is working, not frozen.
...
- [x] **Status bar showing connection to current provider and model** — shipped 2026-04-20 as `APIStatusBar` (window-wide strip pinned below the NavigationSplitView). Left half: provider glyph + colored pill (Connected / Unreachable / Unknown) + live "tail -f" of the most recent API call with in-flight elapsed-seconds timer + green-checkmark / red-✕ completion states. Events come from a `MonitoredLLMClient` decorator applied at every `makeLLMClient` call site so `listModels` / `generateCaption` / `generateText` all flow through; preflight handshakes update the pill via `noteHandshake`. Write-back events are instrumented too (marked `tracksConnection: false` so AppleScript outcomes don't touch the LLM pill). Asset-id prefix shown in the tail so per-photo activity is attributable. Right half: Photos auth + Lock auto-start indicators (idle-sleep icon was added then removed — rarely actionable).
- [x] **Preflight is Ollama-hardcoded when provider is OpenAI-compatible** — shipped 2026-04-20. `PreflightSheet` now takes `provider: LLMProvider` and branches copy, fix-instructions, and the Start-Ollama button on it via a `k()` key-routing helper: Ollama keeps the existing `preflight.*` keys + copy-paste shell commands; OpenAI-compatible gets new `preflight.*_openai` keys + a `FixBullets` block with mlx-vlm / LM Studio / vLLM-agnostic guidance (no canonical CLI). `LibraryWindow` now picks the right `baseURL` (`engine.connection.baseURL` vs `engine.openaiConnection.baseURL`) based on `engine.apiProvider` so the displayed URL matches the probe. `ProcessingEngine.applyConfigChange()` now ends with `await runPreflight()` so stale failure sheets don't linger after a provider/URL/model swap (preflight resets status to `.checking` first, sheet transitions cleanly to `.ok` or re-presents with fresh copy). Added 10 new localization keys across 8 languages (EN/FR/ES/DE/PT/JA/ZH-Hans/KO).
- [ ] **View & clean up accumulated descriptions from multiple runs** — on repeated runs (sentinel bumps, prompt edits, provider switches, re-translations), a single asset's Photos.app description can accumulate multiple PhotoSnail payloads or stale fragments alongside the user's original text. Offer a GUI inspector that scans processed assets, surfaces descriptions with multiple sentinels / repeated payloads / unexpected length, and lets the user preview + clean them up (keep user prefix, collapse to the latest payload, or strip PhotoSnail content entirely). Needs a detection heuristic (count sentinel occurrences, look for duplicated `\n\n---\n\n` separators) and a safe batch-edit path through `PhotosScripter`. Related to the original-description preservation item below but scoped to cleanup rather than prevention.
- [x] **Persist the original (pre-PhotoSnail) description across re-processes** — shipped 2026-04-20. Went with path (a): on every write, the new `Pipeline.splitExistingDescription(_:)` walks segments split by `\n\n---\n\n` and finds the LAST one containing a PhotoSnail sentinel — everything before it is the user's prefix (preserved verbatim), everything from that segment onward is ours (replaced). `formatDescription` reassembles `<user-prefix>\n\n---\n\n<new-payload>`. Covers reprocessing, sentinel bumps, prompt edits, provider switches, and re-translations without ever losing the user's original text. No schema change. Rejected path (b) (snapshot column) because the user may edit the prefix after our first touch and we'd want their live edits to survive, not a frozen first-touch snapshot. 7 new test cases cover pure-ours, reprocess, 3-touch no-accumulation, user-deleted sentinel, multi-separator prefix, plus 4 direct tests on the splitter.
- [x] **Pause vs Stop split with correct auto-start-when-locked semantics** — shipped 2026-04-20. The original premise ("on next lock, the watcher will auto-start anyway") was incorrect — the watcher already respected `.paused` as user-initiated. But the underlying UX question surfaced a cleaner two-action model: Pause (soft) yields to the lock watcher; Stop (hard) opts out entirely. New `EngineState.stopped` + `stopIntent` flag drives the split; worker transitions to `.stopped` and exits cleanly instead of parking. `handleScreenLocked` now auto-resumes from `.paused` when the toggle is on, and early-returns on `.stopped`. Primary button restructured: enabled Start + `LockArmedChip` ("Auto-starts/resumes when Mac locks") replaces the old disabled "Waiting for lock…" label; Running and Paused gain a secondary Stop button. 48 new localized strings across 8 languages (`button.stop`, `status.stopped`, `chip.auto_start_armed`, `chip.auto_resume_armed`).
- [ ] **Post to r/LocalLLaMA** — announcement once happy with a release. Pitch angle: "local-first macOS Photos tagger, Gemma via Ollama, writes descriptions back into Photos.app metadata so they sync via iCloud + show up in Spotlight/Photos search." Include a screenshot + the install instructions from the v0.1.1 release notes. Highest-signal audience for this tool — the LLM-on-device crowd is already running Ollama and will get the pitch instantly.
...
- [ ] **Try Qwen3.6 instead of Gemma4** — benchmark Qwen3.6 against gemma4:31b on the Phase D 20-photo sample and a broader 50+ photo cross-section. Track: quality (spatial reasoning, brand recognition, description specificity), speed (tokens/sec), memory footprint. If Qwen3.6 is ≤90% of gemma4 quality + ≥10% faster, consider a sentinel bump + batch re-process. If ≥95% quality, re-run the full library.
- [x] **Propose shorter sentinel family by default** — shipped 2026-04-18 as part of the per-model-family Settings refactor. New `Sentinel.shortFamily(of:)` strips org prefix (`mlx-community/`, `TheBloke/`), quantization suffixes (`-4bit`, `-q4_K_M`, `-gptq`, `-mlx`, `-bf16`, …), parameter-size suffixes (`-35b`, `-a3b`), and instruction-tuning suffixes (`-instruct`, `-chat`). `propose(...)` uses it for new sentinels. `family(of:)` unchanged so existing persisted sentinels keep parsing, and `propose` accepts both short and long forms as "same family" so a user on the pre-upgrade long-form sentinel isn't prompted to migrate.
- [x] **OpenAI-compatible API backend (mlx-vlm support)** — shipped 2026-04-18. `LLMClient` protocol + `OpenAIClient` in PhotoSnailCore; Settings v2 with `apiProvider` + `openai` block; GUI provider segmented control with local-only banner; CLI `--provider openai` + `--openai-url/--openai-key/--openai-header`. Validated end-to-end against Qwen3.6-35B-A3B-4bit on mlx-vlm (MacBook-Pro-2.localdomain:9090), 43 s for a single image including Vision side-channel. See CLAUDE.md "OpenAI-compatible provider" section for design notes.
...
- Retry policy: 3 attempts max, exponential backoff 10s/30s/60s, applied only to `.llmRequestFailed`. Permanent errors (`.imageLoadFailed`, `.llmResponseParseFailed`) skip retries entirely.
...
### Bugs
- [x] **"Follow processing in grid" toggle doesn't work** — toggle renders and persists but the grid doesn't scroll to the current photo when a batch is running. Investigate: `LibraryGrid`'s `.onChange(of: store.engine?.currentPhotoID)` may not be firing, or `store.followCurrentProcessing` isn't being read correctly after the Phase I changes.
- [x] **Untouched filter shows broken/empty gallery** — selecting "Untouched" in the sidebar when untouched count is 0 should show the "Everything is enumerated" `ContentUnavailableView`, but something is off. Investigate the empty-state logic in `LibraryGrid.gridBody`.
- [x] **Logo rendering artifact** — the new transparent-background pixel snail has a visual issue ("broken a bit"). Investigate at the sidebar scale — might be aliasing from nearest-neighbor downscale at non-integer ratio (source 1170×389 → ~72pt sidebar height = 144 device px, ratio 389/144 ≈ 2.7). May need to re-export the wordmark at a clean integer multiple of the display target.
- [x] **Gallery scrolls on photo selection** — clicking a photo in the grid causes the gallery to shift/scroll unexpectedly. Likely a `ScrollViewReader.scrollTo` firing on selection change, or a layout reflow triggered by the inspector panel appearing/resizing. Investigate the scroll anchoring behaviour in `LibraryGrid`.

### UX fixes
- [x] **BulkActionBar pushes the grid down** — the bar appears/disappears on selection, causing the entire grid to jump. Fix: reserve the bar's height permanently (show it always, disable buttons when nothing is selected), OR use an overlay/floating bar that doesn't participate in the VStack layout.
- [x] **BulkActionBar overflows the panel boundary** — the top row of buttons visually bleeds past the column edge. Investigate: possibly the `.padding(.horizontal, Spacing.lg)` isn't accounting for the NavigationSplitView content column insets. May need to clip or inset differently.
- [x] **DockPhotoCard hover-to-peek appear is too fast** — the processing card's hover preview (popover) disappears too quickly when the mouse leaves the thumbnail. Increase the dismiss delay from 120ms (`120_000_000` nanoseconds) to ~300-400ms so the user has time to glance at the preview without it vanishing mid-saccade.
- [x] **Move "Copy tags" from BulkActionBar to inspector** — the bulk bar's "Copy tags" is rarely useful there. Move it to the inspector's Tags section as a small copy icon button. While at it, add a matching copy icon button to the Description section so the user can copy the description text to the clipboard with one click.
