...
struct LibrarySidebar: View {
    ...
    private static let logoWordmarkDark: NSImage? = {
    ...
    private static let logoWordmarkLight: NSImage? = {
        if let url = Bundle.main.url(forResource: "LogoWordmarkLight", withExtension: "png") {
            return NSImage(contentsOf: url)
        }
        return nil
    }()
    ...
    var body: some View {
        List {
            Section {
                Group {
                    let wordmark = colorScheme == .dark ? Self.logoWordmarkDark : Self.logoWordmarkLight
                    if let wordmark {
...
struct BulkActionBar: View {
    ...
    var body: some View {
        ...
        //   row 1 — Add-all-unprocessed primary action + selection
        //           summary + transient status + Deselect on the right
        //   row 2 — selection-scoped actions (Add to Queue, Process now,
        //           Clear, Export) — only rendered when something is
        //           actually selected so the bar collapses to a single
        //           row at rest
